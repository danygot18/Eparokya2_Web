...
export const EventPostUpdate = () => {
    const { id } = useParams();
    const [name, setName] = useState('');
    const [description, setDescription] = useState('');
    const [categories, setCategories] = useState('');
    const [images, setImages] = useState([]); // To handle new image uploads
    const [existingImages, setExistingImages] = useState([]); // Existing images from the event
    ...
    useEffect(() => {
        ...
        const fetchEvent = async () => {
            try {
                setLoading(true);
                const { data } = await axios.get(`${process.env.REACT_APP_API}/api/v1/admin/eventpost/${id}`, config);
                setName(data.event.name);
                setDescription(data.event.description);
                setExistingImages(data.event.images); // Set existing images
                setLoading(false);
            } catch (error) {
                setLoading(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            await axios.put(`${process.env.REACT_APP_API}/api/v1/admin/eventpost/${id}`, formData, config, {
                headers: {
                        ...
                        <div style={styles.imagePreviewContainer}>
                            {existingImages.map((image, index) => (
                                <div key={index} style={styles.imagePreviewWrapper}>
                                    <img src={image.url || image} alt="Existing Event" style={styles.imagePreview} />
                                    <FaTrash 
...
export default EventPostUpdate;
